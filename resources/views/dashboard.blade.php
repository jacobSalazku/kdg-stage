                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @foreach($jobs as $job)
                        <a href="{{route('edit', ['id' => $job->id])}}" class="mt-4 block w-full p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{$job->title}} - {{$job->user->company}}</h5>
                            ...
                            <p class="font-light text-xs text-gray-700 dark:text-gray-400">{{$job->user->email}}</p>
                            <p class="font-light text-xs text-gray-700 dark:text-gray-400">{{$job->user->phone_number}}</p>
