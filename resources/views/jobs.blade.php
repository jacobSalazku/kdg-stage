                ...
                <div class=" w-full p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @foreach($jobs as $job)
                        <a href="{{route('detail', ['id' => $job->id])}}">
                            <div class=" h-auto  mt-6 rounded border border-kdg-grey  shadow-lg flex flex-col justify-center items-start px-5 pt-10 gap-8 cursor-pointer hover:bg-gray-100 dark:bg-gray-800  dark:hover:bg-gray-70">
                                <h3 class="mb-4 text-3xl font-bold tracking-tight text-deep-black ">{{$job->title}} </h3>
                                <h4 class="mb-4 text-lg font-mium tracking-tight text-deep-black ">{{$job->company}}</h3>
                                <p class="font-normal text-gray-700 dark:text-gray-400">{{ substr(strip_tags($job->description), 0, 250) }} ...</p>
                                <p class="mt-6 text-sm text-p-black dark:text-gray-400">{{__('jobs.posted')}} {{$job->updated_at->format('d-m-y H:i')}}</p>
                            <div>
                        </a>                    
                    @endforeach
